...
function Home() {
  ...
  useEffect(() => {
    ...
    navigator.permissions.query({ name: 'microphone' }).then(result => {
      console.log("🎙️ Mic permission:", result.state)
      if (result.state !== "granted") {
        alert("Microphone access is blocked. Allow it in browser settings.")
      }
    })

    const micWatchdog = setInterval(() => {
      if (!isSpeakingRef.current && !isRecognizingRef.current && recognitionRef.current) {
        console.warn("[WATCHDOG] Mic inactive. Restarting...")
        try {
          recognitionRef.current.start()
        } catch (e) {
          if (e.name !== "InvalidStateError") console.error(e)
    ...
    const startTimeout = setTimeout(() => {
      if (isMounted && !isSpeakingRef.current && !isRecognizingRef.current) {
        try {
          recognition.start()
          console.log("🎤 Mic started on mount")
        } catch (e) {
          if (e.name !== "InvalidStateError") console.error(e)
        }
      }
    ...
    recognition.onend = () => {
      ...
      setTimeout(() => {
        if (!isSpeakingRef.current && !isRecognizingRef.current) {
          try {
            recognition.start()
            console.log("🎤 Mic restarted after end")
          } catch (e) {
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error !== "aborted" && isMounted && !isSpeakingRef.current) {
        setTimeout(() => {
          try {
            recognition.start()
            console.log("🎤 Mic restarted after error")
          } catch (e) {
            if (e.name !== "InvalidStateError") console.error(e)
          }
        }, 1000)
  ...
  // One-time greeting after user clicks anywhere
  useEffect(() => {
    const greet = () => {
    ...
    return () => window.removeEventListener('click', greet)
  }, [])
